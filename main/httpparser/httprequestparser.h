...
public:
    ...
    {
        ParseResult r = consume(req, begin, end);
        // now build the path and query members from the uri
        std::size_t qstart = req.uri.find_first_of('?');
        ...
        {
            // seperate query portion
            req.path = req.uri.substr(0, qstart);
            req.query = req.uri.substr(qstart+1);        
        }
